...
\end{equation}
Letting the signal received by the middle element be $\frac{Ae^{j\omega t}}{R}$ where R is the radial distance from the aperture and A is the field strength of the aperture and can be defined as $A(x)$ representing the field strength as a function of position away from the boresight axis. This results in the total signal received being delayed by the extra distance it must travel. This distance causes a delay defined by: $ \tau=\frac{xsin(\theta)}{c}$ derived from $\Delta = xsin(\theta)$ (since $v=\frac{\Delta x}{\Delta t}, \Delta t = \frac{\Delta x}{v}$).
...
\end{equation}
This angle dependant integral is in the form of a Fourier transform, thus can be computed with an FFT. Working in two dimensions allows a 3D beam pattern to be produced by taking the FFT in both dimensions.
The FFT however, works with the $\omega$ axis which must be translated to an angular axis represented by $\theta$. This can be done by the relation $\omega = \frac{2\pi sin(\theta)}{\lambda}$, when rearranged to find $\theta$ produces $\theta = sin^{-1}(\frac{\omega \lambda}{2\pi})$. Substituting the frequency variable produces $\theta = sin^{-1(}f\lambda)$. Appendix C demonstrates how this axis change is possible for a future implementation of this code, but does not implement it.\\
